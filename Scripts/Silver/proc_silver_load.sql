...
BEGIN
    ...
    BEGIN TRY
		...
		SELECT 
			...
			END AS sls_due_dt,
			CASE 
				WHEN sls_sales IS NULL OR sls_sales <= 0 OR sls_sales != sls_quaintity * ABS(sls_price) 
		...
		SELECT
			...
			END AS cid, 
			CASE
				WHEN c_birthdate > GETDATE() THEN NULL
				ELSE c_birthdate
			END AS bdate, -- Set future birthdates to NULL
			CASE
				WHEN UPPER(TRIM(c_gender)) IN ('F', 'FEMALE') THEN 'Female'
